...
|---|---|---|---|
| Population 1970-79 | Census Bureau, Preliminary Estimates of the Intercensal Population of Counties 1970-1979 (issued April 1982), five NYC counties summed | `e7079co.txt` | Rounded to hundreds. The 1970 figure in the file is a modified count of 7,895,563; the page uses the published 1970 count of 7,894,862. |
| Population 1980-89 | Census Bureau, Intercensal Estimates of the Resident Population of States and Counties 1980-1989 (issued March 1992) | `e8089co.txt` | |
| Population 1990-99 | Census Bureau, 1990-2000 intercensal county estimates by age, sex, race and Hispanic origin, via `api.census.gov/data/1990/pep/int_charagegroups`, summed over all cells | `int_charagegroups_1990s.json` | Consistent with both the 1990 and 2000 counts. |
| Population 2000-09 | Census Bureau, Intercensal Estimates of the Resident Population for Counties, April 1, 2000 to July 1, 2010 | `co-est00int-tot.csv` | Consistent with both the 2000 and 2010 counts. |
| Population 2010-19 | Census Bureau, vintage 2020 county estimates (`co-est2020-alldata.csv`), adjusted (see 2.2) | `co-est2020-alldata.csv` | The Bureau has not published 2010-2020 intercensal county estimates. |
| Population 2020-25 | Census Bureau, vintage 2025 county estimates (released June 2026) | `co-est2025-alldata.csv` | Postcensal; 2020 uses the census count. |
| Census counts | 1970: 7,894,862; 1980: 7,071,639; 1990: 7,322,564; 2000: 8,008,278; 2010: 8,175,133; 2020: 8,804,190 | | April 1 counts. |
| Births and deaths 1970-2023 | NYC DOHMH, Summary of Vital Statistics 2023, Table PC1 "Population, Live Births, Fertility Rates, Marriages, Deaths, and Infant Mortality, New York City, 1898-2023" | `dohmh_2023sum_tablePC1_page53.txt` (text of page 53 of `2023sum.pdf`) | Births and deaths to city residents. 1966-70, 1971-75 and 1976-80 are published as five-year averages; the page assigns each year in those spans the average (1970 uses the 1966-70 average). 2001 deaths include the World Trade Center deaths (62,964). |
...
| Census Bureau components of change | CO-2000-8 (1990-2000 county components, `2000c8_36.txt`); vintage 2009 (`co-est2009-alldata.csv`); vintage 2020; vintage 2025; comp8090 (1980-90 county components, `comp8090.txt`) | | Used only for the international/domestic split and cross-checks. All are July-to-June years except comp8090. |
| Arrivals 2006-24 | Census Bureau, American Community Survey 1-year, table B07001 (Geographic Mobility in the Past Year by Age), New York city, plus B05002, B01001, B01002 | `acs/acs1_YYYY.json` | Population aged 1 and over. No 2020 1-year release. The 2005 ACS used a different table layout and is not used. |
| Inter-borough share of in-state movers | Census Bureau, ACS county-to-county migration flows, vintages 2010 (2006-2010) through 2020 (2016-2020), five NYC counties | `flows/flows_YYYY.json` | Later vintages publish only state-level rows. |
| Arrivals 1995-2000 | 2000 census, county-to-county migration flow files (inflow and outflow), NYC rows only; 2000 Summary File 3 table P24 (Residence in 1995), New York city | `c2c2000_inflow_nyc.txt`, `c2c2000_outflow_nyc.txt`, `sf3_2000_nyc_mobility_nativity.json` | People aged 5 and over. |
| Age structure | Census Bureau county estimates by age: `co-asr-7079-nyc.csv` (1970 modified count), `pe-02-nyc.csv` (July 1, 1980), the 1990s API file (July 1, 1990), `co-est00int-alldata-36.csv` (April 1, 2000 count), `cc-est2020-alldata-36.csv` (April 1, 2010 count), `cc-est2025-alldata-36.csv` (April 1, 2020 estimates base and July 1, 2025 estimate) | | |
| Race and Hispanic origin 1970-2020 | Decennial census: Gibson and Jung, Census Bureau Population Division Working Paper 76 (2005) for 1970-1990; Summary File 1 and P.L. 94-171 files for 2000-2020, as compiled for the Vital City "Who lives here" chart | `../nyc-demographics-horserace/data/data.json` | Shares as published; race is "race alone." |
| Foreign-born 1970-2000 | NYC Department of City Planning, The Newest New Yorkers, 2013 edition, Table 2-1 (decennial census figures) | `dcp_nny2013_table2-1_page20.txt` | The 2000 figure (2,871,032) ties to 2000 SF3 table P21. |
...
### 2.2 Adjusting the 2010-19 estimates to the 2020 count

The vintage 2020 series put the July 1, 2020 population at 8,253,213; the 2020 census counted 8,804,190. The Bureau has not published a reconciled ("intercensal") county series for 2010-2020. The page therefore adds the 550,977 difference to the vintage 2020 estimates in proportion to elapsed time, in the spirit of the Bureau's own intercensal method: `adjusted(y) = estimate(y) + 550,977 × (y − 2010) / 10` for 2011-2019. The 2020 Post-Enumeration Survey found that New York State was overcounted by about 3.4 percent, so the truth for the late 2010s probably lies between the two series. Every downstream figure for the 2010s (annual net migration, the decade ledger, the calculator) inherits this choice.
...
- the "back-of-the-envelope" version: `(births + arrivals over the window) / population(B)`, which is what a quick tally gives and which ignores newcomers who later left or died. For 2001-2013 it returns 51 percent, matching the "fully half" figure the Department of City Planning gave Harry Siegel in 2014; the cohort version returns 42 percent. For 2001-2025 the simple tally exceeds 100 percent, which is why the cohort version is the headline.
...
- The 2010s depend on the adjustment in 2.2; the 2020-25 estimates are postcensal, contested by the Department of City Planning for 2021-22, and will be revised.
...
- Race categories changed in 2000 (multiple-race answers) and 2020 (coding of write-in answers), and the 1970 census undercounted the foreign-born relative to later methods.
